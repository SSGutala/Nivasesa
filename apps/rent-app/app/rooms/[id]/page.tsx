...
import { getRoomListingById, applyToRoom, respondToApplication } from '@/actions/rooms';
...
export default function RoomDetailPage() {
    ...
    const handleRespond = async (applicationId: string, status: 'accepted' | 'rejected') => {
        ...
        if (result.success) {
            ...
            const data = await getRoomListingById(params.id as string);
            setListing(data);
            setFeedback({ type: 'success', message: `Application ${status}` });
        } else {
                    ...
                    {listing.amenities && (
                        <section className={styles.section}>
                            ...
                            <div className={styles.amenities}>
                                {listing.amenities.split(',').map((amenity: string, idx: number) => (
            ...
            {listing.applications && listing.applications.length > 0 && (
                <section className={styles.applicationsSection}>
                    ...
                    <div className={styles.applicationsList}>
                        {listing.applications.map((app: typeof listing.applications[0]) => (
