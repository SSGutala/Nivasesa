...
export async function getConversationsAction(): Promise<{
  ...
  try {
    ...
    const conversationsWithDetails: ConversationWithDetails[] = await Promise.all(
      conversations.map(async (conv: typeof conversations[0]) => {
        const otherUserId =
...
export async function getMessagesAction(
  ...
  try {
    ...
    const userIds = [...new Set(messages.map((m: typeof messages[0]) => m.senderId))]
...
export async function getUnreadCountAction(): Promise<{
  ...
  try {
    ...
    const conversationIds = conversations.map((c: typeof conversations[0]) => c.id)
