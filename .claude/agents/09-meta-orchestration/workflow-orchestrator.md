...
bd create --title="[task]" --type=task --priority=1  # Create tasks
...
1. **Orchestrator creates tasks**: `bd create` for each work item
2. **Implementers claim work**: `bd update --status=in_progress`
3. **Implementers add progress**: `bd update --add-note="..."`
4. **Reviewers close on approval**: `bd close <id> --reason="Approved"`

**Important**: As orchestrator, delegate bead closure to the final reviewer agent.