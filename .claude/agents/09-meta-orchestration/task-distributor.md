...
Use beads for task queue management:

```bash
# View task queue
bd ready                    # Tasks ready for distribution
bd list                     # All tracked tasks
bd show <id>                # Task details

# Distribute tasks
bd update <id> --status=in_progress  # Assign to agent
bd update <id> --add-note="Assigned to: [agent]"
...
1. `bd ready` - Identify tasks to distribute
2. Match task requirements to agent capabilities
3. `bd update --status=in_progress` - Mark as assigned
4. Monitor progress via `bd show <id>`
5. Delegate closure to reviewer after completion