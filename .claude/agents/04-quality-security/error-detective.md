...
Track debugging and error investigation:
...
bd update <issue-id> --add-note="Root cause: [description]"
bd update <issue-id> --add-note="Fix applied: [summary]"

# Sync state
bd sync
```

**Note**: Do NOT close beads. Report fix details and let the code-reviewer close on final approval.
